...
from sports_signal_bot.training.leakage import (
    audit_feature_target_alignment, enforce_pre_match_only_feature_policy)
    ...
    def build(
        ...
        if "label_name" in labels_df.columns:
            filtered_labels = labels_df[
                labels_df["label_name"] == self.config.label_name
            ].copy()
        else:
        ...
        if filtered_labels.empty:
            raise ValueError(
                f"No labels found for label_name: {self.config.label_name}"
            )

        # Drop invalid labels unless explicitly permitted
        if (
            "validity_status" in filtered_labels.columns
            and not self.config.allow_missing_labels
        ):
            # Assumes LabelValidityStatus.VALID corresponds to some string, e.g., 'valid'
            # Check if using enum directly or string
            valid_mask = filtered_labels["validity_status"].astype(
                str
            ).str.lower().str.endswith("valid") & ~filtered_labels[
                "validity_status"
            ].astype(
                str
        ...
        if (
            "target_value" not in filtered_labels.columns
            and "class_index" not in filtered_labels.columns
        ):
        ...
        target_col = (
            "class_index"
            if "class_index" in filtered_labels.columns
            else "target_value"
        )
        ...
        if target_col in features_df.columns:
            raise LeakageDetectedError(
                f"Target column '{target_col}' already exists in feature matrix. This is data leakage."
            )

        # Join features and labels
        df = pd.merge(
            features_df,
            filtered_labels[["event_id", target_col]],
            on="event_id",
            how="inner",
        )
        ...
        if "event_datetime_utc" not in df.columns:
            logger.warning(
                "No event_datetime_utc in dataset, adding a placeholder. Time-aware features might fail."
            )
        ...
        if not df.empty and df[target_col].dtype.kind in "biu":
            summary.class_distribution = {
                str(k): v for k, v in df[target_col].value_counts().to_dict().items()
            }
