...
def evaluate_classification_metrics(y_true: np.ndarray, y_pred_proba: np.ndarray, y_pred: np.ndarray, classes: np.ndarray) -> Dict[str, float]:
    ...
    if len(classes) == 2:
        ...
        if y_pred_proba.ndim == 2 and y_pred_proba.shape[1] == 2:
            ...
            try:
                metrics['brier_score'] = float(brier_score_loss(y_true, prob_pos, pos_label=classes[1]))
            except ValueError:
                metrics['brier_score'] = float('nan')

            metrics['f1'] = float(f1_score(y_true, y_pred, pos_label=classes[1], zero_division=0))
    else:
        # Multiclass F1
