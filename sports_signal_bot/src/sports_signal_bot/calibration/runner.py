...
class CalibrationRunner:
    def __init__(self, config: Dict[str, Any]):
        ...
        # Path setup
        self.output_dir = get_project_root() / "data" / "processed" / "calibration" / self.sport / self.market_type / self.run_id
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def _evaluate(self, X: np.ndarray, y: np.ndarray, method: str) -> CalibrationSummary:
        """Evaluates metrics and reliability bins for a probability matrix."""

        is_binary = len(self.class_labels) == 2
        positive_class_index = 1 if is_binary else -1 # Unused in multiclass ECE fallback generally

        log_loss_val = calculate_log_loss(y, X, labels=np.arange(len(self.class_labels)))
        brier_val = calculate_brier_score(y, X, positive_class_index=positive_class_index)
        ece, mce = calculate_ece_mce(y, X, n_bins=self.config.get("n_bins", 10), positive_class_index=positive_class_index)

        bins = generate_reliability_bins(y, X, n_bins=self.config.get("n_bins", 10), positive_class_index=positive_class_index)
    ...
    def run(self, validation_records: List[ValidationPredictionRecord]) -> Dict[str, Any]:
        logger.info(f"Starting calibration run {self.run_id} using method {self.method_name}")
        ...
        # 1. Build Dataset
        df = build_calibration_dataset_from_validation_predictions(validation_records, self.class_labels)
        ...
        if comparison.possible_overfit_warning:
            warnings.append("Possible overfit detected: ECE is extremely low compared to raw.")
        if not comparison.calibration_improvement:
            warnings.append("Calibration did not improve log loss or ECE significantly.")

        # 7. Generate Output Records
        calibrated_records = []
        for i, row in df.iterrows():
            event_id = str(row["event_id"])
            raw_probs = {label: X_raw[i, j] for j, label in enumerate(self.class_labels)}
            cal_probs = {label: X_calibrated[i, j] for j, label in enumerate(self.class_labels)}
            cal_class = int(np.argmax(X_calibrated[i]))

            # Match back to original record model name and fold if possible
            orig_rec = next((r for r in validation_records if r.event_id == event_id), None)
        ...
        with open(bins_path, "w") as f:
            json.dump([b.model_dump() for b in calibrated_summary.reliability_bins], f, indent=2)
