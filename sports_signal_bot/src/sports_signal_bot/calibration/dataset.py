...
def build_calibration_dataset_from_validation_predictions(
    validation_records: List[ValidationPredictionRecord], class_labels: List[str]
) -> pd.DataFrame:
    """
    ...
    for record in validation_records:
        ...
        for i, class_label in enumerate(class_labels):
            row[f"prob_{class_label}"] = record.predicted_probabilities.get(
                class_label, 0.0
            )

        if record.true_class_index is not None and 0 <= record.true_class_index < len(
            class_labels
        ):
