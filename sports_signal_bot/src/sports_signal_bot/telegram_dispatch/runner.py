...
class DispatchRunner:
    def __init__(
        ...
        self.routing_policy = TelegramRoutingPolicy(channels_config, routing_config, env_config)
        ...
        # Only initialize client if we have a token (or if not strictly dry run, but good practice anyway)
        token = env_config.get("TELEGRAM_BOT_TOKEN")
        if not token and not self.dry_run:
            logger.warning("TELEGRAM_BOT_TOKEN is missing. Forcing dry_run=True.")
            self.dry_run = True

        self.client = TelegramClient(bot_token=token if token else "dummy")
        self.sender = TelegramSender(self.client, self.routing_policy, delivery_config)

    def process_decisions(self, run_id: str, slot: str, mode: str, payload_records: List[DispatchPayloadRecord]) -> TelegramDispatchManifest:
        ...
        for payload in payload_records:
            ...
            if payload.decision_class == "approved":
                ...
                severity = MessageSeverity.INFO
                review_record = self.review_builder.build_from_candidate(payload)
                body = self.template_engine.render_review(review_record)
            else:
                 continue # Skip non-actionable like "watchlist" for direct alerts (they go to summary)

            # Route
            routing_decision = self.router.route_message(msg_id, msg_type, severity, inference_mode=mode)
    ...
    def process_summary(self, run_id: str, slot: str, mode: str, summary_record: SummaryMessageRecord) -> TelegramDispatchManifest:
        manifest = TelegramDispatchManifest(run_id=run_id)
        msg_id = f"msg_{uuid.uuid4().hex[:8]}"
    ...
    def process_alarm(self, run_id: str, slot: str, mode: str, alarm_record: AlarmMessageRecord) -> TelegramDispatchManifest:
        manifest = TelegramDispatchManifest(run_id=run_id)
        msg_id = f"msg_{uuid.uuid4().hex[:8]}"
        msg_type = MessageType.CRITICAL_ALARM if alarm_record.severity == MessageSeverity.CRITICAL else MessageType.PIPELINE_WARNING

        body = self.template_engine.render_alarm(alarm_record)
        routing_decision = self.router.route_message(msg_id, msg_type, alarm_record.severity, inference_mode=mode)
    ...
    def _execute_delivery(self, messages: List[TelegramMessageRecord], manifest: TelegramDispatchManifest):
        ...
        for msg in messages:
            ...
            if self.dry_run:
                ...
                record = TelegramDispatchRecord(message=msg, final_status=DeliveryStatus.DRY_RUN_ONLY)
                manifest.records.append(record)
                logger.info(f"[DRY RUN] Would send to {msg.channel_name}:\n{msg.body}")
                continue

            if msg.delivery_status == DeliveryStatus.SUPPRESSED:
                manifest.suppressed_count += 1
                record = TelegramDispatchRecord(message=msg, final_status=DeliveryStatus.SUPPRESSED)
                manifest.records.append(record)
                continue

            attempts = self.sender.send(msg, parse_mode=self.parse_mode)
            final_status = attempts[-1].status if attempts else DeliveryStatus.FAILED_FINAL
            msg.delivery_status = final_status

            record = TelegramDispatchRecord(message=msg, attempts=attempts, final_status=final_status)
            manifest.records.append(record)

            if final_status == DeliveryStatus.SENT:
                manifest.total_messages_sent += 1
            elif final_status in [DeliveryStatus.FAILED_FINAL, DeliveryStatus.FAILED_RETRYABLE]:
                manifest.final_failures_count += 1

            manifest.retry_count += max(0, len(attempts) - 1)

            # Update breakdowns
            manifest.channel_breakdown[msg.channel_name] = manifest.channel_breakdown.get(msg.channel_name, 0) + 1
            manifest.severity_breakdown[msg.severity.value] = manifest.severity_breakdown.get(msg.severity.value, 0) + 1
