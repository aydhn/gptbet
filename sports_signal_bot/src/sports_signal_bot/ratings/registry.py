...
from sports_signal_bot.ratings.glicko_placeholder import \
    GlickoPlaceholderEngine


class RatingEngineRegistry:
    def __init__(self):
        self._engines: Dict[str, Type[BaseRatingEngine]] = {
            "elo": EloRatingEngine,
            "glicko": GlickoPlaceholderEngine,
        }
    ...
    def get_engine_class(self, name: str) -> Type[BaseRatingEngine]:
        ...
        if name_lower not in self._engines:
            raise ValueError(f"Unknown rating engine: {name}")
