...
def get_margin_multiplier(method: str, home_score: float, away_score: float, rating_diff: float = 0.0, cap: Optional[float] = None) -> float:
    if method == "no_margin": return 1.0
    margin = abs(home_score - away_score)
    if margin == 0: return 1.0
    if method == "log_margin": return max(1.0, math.log(margin + 1))
