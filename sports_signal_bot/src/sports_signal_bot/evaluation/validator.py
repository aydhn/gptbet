...
def validate_evaluation_data(df: pd.DataFrame, required_cols: List[str]) -> List[str]:
    ...
    if "true_label" in df.columns:
        ...
        if missing_labels > 0:
            warnings.append(f"Found {missing_labels} rows with missing true labels. They will be ignored.")

    if "predicted_class" in df.columns:
        missing_preds = df["predicted_class"].isna().sum()
        if missing_preds > 0:
            warnings.append(f"Found {missing_preds} rows with missing predicted classes.")
