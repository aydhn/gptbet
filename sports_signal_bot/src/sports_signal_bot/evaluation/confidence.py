...
def build_confidence_buckets(
    ...
    df_eval = pd.DataFrame(
        {"true_label": y_true, "pred_class": y_pred_class, "confidence": confidences}
    )
    ...
    for i, col in enumerate(proba_cols):
        df_eval[f"prob_{i}"] = (
            y_pred_proba[:, i]
            if y_pred_proba.shape[1] > 1
            else (y_pred_proba.flatten() if i == 0 else 1 - y_pred_proba.flatten())
        )

    # Create bins
    bins_edges = np.linspace(
        1.0 / (len(proba_cols) if y_pred_proba.shape[1] > 1 else 2), 1.0, bins + 1
    )
    df_eval["bucket"] = pd.cut(
        df_eval["confidence"], bins=bins_edges, include_lowest=True
    )
