...
def compute_pairwise_comparison(
    ...
    common_events = set(df_a[event_id_col].unique()).intersection(
        set(df_b[event_id_col].unique())
    )

    if len(common_events) == 0:
        return None

    df_a_common = df_a[df_a[event_id_col].isin(common_events)].sort_values(
        by=event_id_col
    )
    df_b_common = df_b[df_b[event_id_col].isin(common_events)].sort_values(
        by=event_id_col
    )
    ...
    delta_log_loss = metrics_a.get("log_loss", np.nan) - metrics_b.get(
        "log_loss", np.nan
    )
    delta_brier = metrics_a.get("brier", np.nan) - metrics_b.get("brier", np.nan)
    delta_accuracy = metrics_a.get("accuracy", np.nan) - metrics_b.get(
        "accuracy", np.nan
    )
