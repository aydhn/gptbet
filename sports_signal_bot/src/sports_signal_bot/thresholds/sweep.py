...
import pandas as pd
from .contracts import ThresholdCandidateRecord, ThresholdOptimizationResult, ThresholdSweepRecord, ThresholdPolicyRecord
...
class ThresholdSweepEngine:
    ...
    def sweep(self, strategy_name: str, signals: List[SignalScoreRecord], labels_df: pd.DataFrame, sport: str, market_type: str, true_label_col: str = "target_value", class_labels: Optional[List[str]] = None) -> ThresholdOptimizationResult:
        grid = generate_threshold_grid(self.config.get("grid", {}), strategy_name)

        from .factory import ThresholdStrategyFactory
        strategy = ThresholdStrategyFactory.create(strategy_name, self.config.get("strategy_config", {}))
        ...
        for params in grid:
            ...
            if accepted:
                ...
                eval_df = labels_df[labels_df['event_id'].isin(accepted_event_ids)].copy()

                if not eval_df.empty:
                    # We need to structure the predictions to compute metrics
                    # Create predicted_class and probabilities
                    eval_df['predicted_class'] = [next(s.selection for s in accepted if s.event_id == eid) for eid in eval_df['event_id']]
                    ...
                    if true_label_col in eval_df.columns:
                        ...
                        try:
                            ...
                            cand.quality_metrics["accuracy"] = float(accuracy_score(y_true, y_pred))
                        except Exception:
                            cand.quality_metrics["accuracy"] = 0.0

                cand.average_signal_score = sum(s.final_signal_score for s in accepted) / len(accepted)
                cand.average_edge = sum(s.components.edge_estimate for s in accepted) / len(accepted)
                cand.average_confidence = sum(s.components.confidence_score for s in accepted) / len(accepted)
                cand.average_uncertainty_penalty = sum(s.components.uncertainty_penalty for s in accepted) / len(accepted)

            else:
                cand.warnings.append("Empty accepted set")

            cand.objective_value = self.objective_evaluator.evaluate(cand, cand.quality_metrics)
        ...
        return ThresholdOptimizationResult(
            ...
            warnings=["No valid candidates after constraints"] if not best_candidate else []
        )
