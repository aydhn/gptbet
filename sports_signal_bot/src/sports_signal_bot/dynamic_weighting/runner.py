...
from sports_signal_bot.dynamic_weighting.contracts import (
    DynamicWeightRecord, WeightingDiagnosticsRecord, WeightingDecisionRecord
)
from sports_signal_bot.dynamic_weighting.factory import WeightingFactory

class DynamicWeightingRunner:
    def __init__(self, strategy_name: str, policy_data: Dict[str, Any], config: Dict[str, Any]):
        self.strategy = WeightingFactory.create(strategy_name, policy_data, config)
        self.strategy_name = strategy_name

    def run(self, eligible_sources: List[Dict[str, Any]], context: Dict[str, Any]) -> Tuple[List[DynamicWeightRecord], WeightingDiagnosticsRecord]:
    ...
    def _generate_diagnostics(self, event_id: str, weights: List[DynamicWeightRecord], sources: List[Dict[str, Any]]) -> WeightingDiagnosticsRecord:
        capped = []
        floored = []
        stale = []
        trust_sum = 0.0
