...
from sports_signal_bot.dynamic_weighting.components import compute_base_prior, compute_trust_weight_component, combine_weight_components
from sports_signal_bot.dynamic_weighting.normalization import normalize_weights, explain_weighting_decision

class StaticPolicyWeighted(BaseWeightingStrategy):
    def compute_weights(self, eligible_sources: List[Dict[str, Any]], context: Dict[str, Any]) -> List[DynamicWeightRecord]:
        ...
        for source in eligible_sources:
            ...
            # Simple policy: only trust + prior
            components = combine_weight_components(
                trust=trust, regime=0.0, disagreement=0.0, recency=0.0, health=1.0,
                prior=prior, bonus=0.0, policy=self.policy
            )
