...
def compute_base_prior(
    source_family: str, market_type: str, config: Dict[str, Any]
) -> float:
...
def compute_regime_weight_component(
    regime_fit: float, sample_size: int, damping_factor: float = 0.5
) -> float:
...
def compute_disagreement_component(
    source_prob: float,
