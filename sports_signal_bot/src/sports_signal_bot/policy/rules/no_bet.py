...
class ScoreGrayZoneRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if min_s <= signal.final_signal_score < max_s:
            return True, [DecisionRationaleRecord(code="no_bet_gray_zone", description=f"Score {signal.final_signal_score:.2f} in gray zone [{min_s}, {max_s})", impact="negative")]
        return False, []
...
class HighUncertaintyRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if u > max_u:
            return True, [DecisionRationaleRecord(code="high_uncertainty", description=f"Uncertainty {u:.2f} > {max_u}", impact="negative")]
        return False, []
...
class HighDisagreementRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if d > max_d:
            return True, [DecisionRationaleRecord(code="high_disagreement", description=f"Disagreement {d:.2f} > {max_d}", impact="negative")]
        return False, []
...
class InsufficientEdgeRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if signal.edge_estimate < min_edge:
            return True, [DecisionRationaleRecord(code="insufficient_edge", description=f"Edge {signal.edge_estimate:.3f} < {min_edge}", impact="negative")]
        return False, []
