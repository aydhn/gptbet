...
class CandidateScoreRule(BasePolicyRule):
    def evaluate(
        ...
        min_c = self.config.get("score_bands", {}).get(
            "watchlist", 0.6
        )  # Just above watchlist
        if signal.final_signal_score >= min_c:
            return True, [
                DecisionRationaleRecord(
                    code="candidate_score",
                    description=f"Score {signal.final_signal_score:.2f} >= candidate min {min_c}",
                    impact="positive",
                )
            ]
        return False, []
...
class CandidateRuleSet:
    ...
    def evaluate(
        ...
        if is_candidate:
            all_reasons.append(
                DecisionRationaleRecord(
                    code="candidate_but_not_approved",
                    description="Candidate conditions met but lacking strong approval",
                    impact="neutral",
                )
            )
