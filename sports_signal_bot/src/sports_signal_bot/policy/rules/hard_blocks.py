        ...
        if signal.final_probability < 0.0 or signal.final_probability > 1.0:
            return True, [
                DecisionRationaleRecord(
                    ...
                    description="Probability outside [0, 1]",
                    impact="blocking",
                )
            ]
        return False, []
...
class MissingMarketReferenceRule(BasePolicyRule):
    def evaluate(
        ...
        has_ref = (
            signal.components_summary.get("market_implied_probability") is not None
        )
        if req and not has_ref:
            return True, [
                DecisionRationaleRecord(
                    code="missing_market_reference",
                    description="Required market reference is missing",
                    impact="blocking",
                )
            ]
        return False, []
...
class LowDataQualityBlockRule(BasePolicyRule):
    def evaluate(
        ...
        if quality < min_dq:
            return True, [
                DecisionRationaleRecord(
                    code="too_low_data_quality",
                    description=f"Quality {quality:.2f} < {min_dq:.2f}",
                    impact="blocking",
                )
            ]
        return False, []
