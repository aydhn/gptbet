...
class StrongScoreRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if signal.final_signal_score >= min_s:
            return True, [DecisionRationaleRecord(code="passed_score_threshold", description=f"Score {signal.final_signal_score:.2f} >= {min_s}", impact="positive")]
        return False, []
...
class StrongEdgeRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if signal.edge_estimate >= min_e:
            return True, [DecisionRationaleRecord(code="passed_edge_threshold", description=f"Edge {signal.edge_estimate:.3f} >= {min_e}", impact="positive")]
        return False, []
...
class LowUncertaintyRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if u <= max_u:
            return True, [DecisionRationaleRecord(code="low_uncertainty", description=f"Uncertainty {u:.2f} <= {max_u}", impact="positive")]
        return False, []
...
class LowDisagreementRule(BasePolicyRule):
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if d <= max_d:
            return True, [DecisionRationaleRecord(code="low_disagreement", description=f"Disagreement {d:.2f} <= {max_d}", impact="positive")]
        return False, []
...
class ApprovalRuleSet:
    ...
    def evaluate(self, signal: SignalPolicyInputRecord) -> Tuple[bool, List[DecisionRationaleRecord]]:
        ...
        if is_approved:
            all_reasons.append(DecisionRationaleRecord(code="approved_by_strong_signal", description="All approval conditions met", impact="positive"))
