...
class PolicyRunner:
    ...
    def evaluate_signal(self, signal: SignalPolicyInputRecord) -> PolicyDecisionRecord:
        ...
        if is_blocked:
            ...
            return self._build_decision(
                signal, PolicySignalStatus.BLOCKED, rationales, hr_reasons
            )
        ...
        if any(r.impact == "blocking" for r in rationales):
            return self._build_decision(
                ...
                rationales,
                [r for r in rationales if r.impact == "blocking"],
            )
        ...
        if is_no_bet:
            ...
            if signal.final_signal_score < self.config.get("score_bands", {}).get(
                "no_bet", 0.4
            ):
                return self._build_decision(
                    signal, PolicySignalStatus.WEAK_SIGNAL, rationales, nb_reasons
                )
            return self._build_decision(
                signal, PolicySignalStatus.NO_BET_ZONE, rationales, nb_reasons
            )
        ...
        if is_candidate:
            ...
            if is_approved:
                ...
                return self._build_decision(
                    signal, PolicySignalStatus.APPROVED, rationales, []
                )
            else:
                return self._build_decision(
                    signal, PolicySignalStatus.CANDIDATE, rationales, []
                )

        if is_watchlist:
            rationales.extend(w_reasons)
            return self._build_decision(
                signal, PolicySignalStatus.BELOW_THRESHOLD, rationales, []
            )
    ...
    def _build_decision(
        ...
        blocking_or_no_bet_reasons: List[DecisionRationaleRecord],
    ) -> PolicyDecisionRecord:
        ...
        return PolicyDecisionRecord(
            ...
            no_bet_reasons=[
                r.code for r in blocking_or_no_bet_reasons if r.impact == "negative"
            ],
            block_reasons=[
                r.code for r in blocking_or_no_bet_reasons if r.impact == "blocking"
            ],
            warnings=[],
        )

    def run(
        self, signals: List[SignalPolicyInputRecord], sport: str, market: str
    ) -> PolicyManifest:
        decisions = []
        lifecycles = []
        ...
        for d in decisions:
            if d.signal_status == PolicySignalStatus.APPROVED:
                manifest.approved_count += 1
            elif d.signal_status == PolicySignalStatus.CANDIDATE:
                manifest.candidate_count += 1
            elif d.action_class == ActionClass.WATCHLIST:
                manifest.watchlist_count += 1
            elif d.signal_status == PolicySignalStatus.BLOCKED:
                manifest.blocked_count += 1
            elif d.action_class == ActionClass.NO_ACTION:
                manifest.no_action_count += 1

            for code in d.rationale_codes:
                manifest.top_rationale_codes[code] = (
                    manifest.top_rationale_codes.get(code, 0) + 1
                )
            for code in d.no_bet_reasons:
                manifest.no_bet_zone_reasons[code] = (
                    manifest.no_bet_zone_reasons.get(code, 0) + 1
                )
            for code in d.block_reasons:
                manifest.blocked_reasons[code] = (
                    manifest.blocked_reasons.get(code, 0) + 1
                )
