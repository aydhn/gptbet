...
def determine_band(value: float, bands_config: Dict[str, float], reverse: bool = False) -> str:
...
def get_score_band(score: float, config: Dict[str, Any]) -> str:
    ...
    if score < bands.get("rejected", 0.2): return "rejected"
    if score < bands.get("no_bet", 0.4): return "no_bet"
    if score < bands.get("watchlist", 0.6): return "watchlist"
    if score < bands.get("candidate", 0.8): return "candidate"
    return "approved"
...
def get_edge_band(edge: float, config: Dict[str, Any]) -> str:
    ...
    if edge < bands.get("low", 0.01): return "none"
    if edge < bands.get("medium", 0.03): return "low"
    if edge < bands.get("high", 0.05): return "medium"
    return "high"
