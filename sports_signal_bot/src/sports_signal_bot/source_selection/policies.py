...
from .contracts import (SourceEligibilityRecord, SourceExclusionReasonRecord,
...
class BasePolicy:
    ...
    def evaluate_group(
        self, records: List[SourceEligibilityRecord], context: Dict[str, Any]
    ) -> None:
        pass  # Base implementation does nothing
...
class BasicAvailabilityPolicy(BasePolicy):
    def evaluate(
        ...
        if not metadata.is_prediction_available:
            ...
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    ...
                    details="Prediction not available for event.",
                )
            )
        ...
        if metadata.has_invalid_probabilities:
            ...
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    reason_code="invalid_probabilities",
                    details="NaNs or out of bounds prob detected.",
                )
            )
...
class QualityThresholdPolicy(BasePolicy):
    def evaluate(
        ...
        if not self.is_enabled or not eligibility.is_eligible:
            return
        ...
        if (
            ...
            and eligibility.trust_score.total_trust_score < min_trust
        ):
            eligibility.is_eligible = False
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    reason_code="low_trust_score",
                    details=f"Trust score below {min_trust}",
                )
            )
        ...
        if metadata.recent_coverage_rate < min_cov:
            ...
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    reason_code="insufficient_recent_coverage",
                    details=f"Coverage below {min_cov}",
                )
            )
        ...
        if metadata.model_age_days > max_model_age:
            ...
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    reason_code="stale_model",
                    details=f"Model age > {max_model_age} days",
                )
            )
...
class RegimeAwarePolicy(BasePolicy):
    def evaluate(
        ...
        if eligibility.trust_score and eligibility.trust_score.regime_fit_score < 0.2:
            ...
            eligibility.exclusion_reasons.append(
                SourceExclusionReasonRecord(
                    reason_code="low_regime_evidence", details="Very poor regime fit."
                )
            )
...
class PreferredCalibratedPolicy(BasePolicy):
    def evaluate_group(
        ...
        for family, fam_records in families.items():
            has_calibrated = any(
                context.get("metadata_map", {})[r.source_name].is_calibrated
                ...
                for r in fam_records:
                    if not context.get("metadata_map", {})[r.source_name].is_calibrated:
                        ...
                        r.exclusion_reasons.append(
                            SourceExclusionReasonRecord(
                                reason_code="replaced_by_calibrated_variant",
                                details="Calibrated variant preferred.",
                            )
                        )
...
class FallbackSafetyPolicy(BasePolicy):
    def evaluate_group(
        ...
        if eligible_count < min_required:
            ...
            fallback_sources = self.definition.parameters.get(
                "fallback_source_priority", []
            )
            for r in records:
                if not r.is_eligible and (
                    r.source_name in fallback_sources
                    or r.source_family in fallback_sources
                ):
                    # Rescue if not fundamentally broken
                    fatal_reasons = {"source_unavailable", "invalid_probabilities"}
                    if not any(
                        ex.reason_code in fatal_reasons for ex in r.exclusion_reasons
                    ):
                        r.is_eligible = True
