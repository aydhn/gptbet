...
try:
    from sports_signal_bot.dynamic_weighting.contracts import (
        WeightingDecisionRecord, WeightingManifest)
    from sports_signal_bot.dynamic_weighting.manifests import \
        save_weighting_manifest
    from sports_signal_bot.dynamic_weighting.registry import weighting_registry
    from sports_signal_bot.dynamic_weighting.runner import \
...
def _get_mock_eligible_sources(sport: str, market: str) -> List[dict]:
    ...
    if sport == "football" and market == "1x2":
        return [
            ...
            {
                "name": "log_reg_1",
                "family": "ml_calibrated",
                "trust_score": 0.9,
                "regime_fit": 0.8,
                "regime_sample_size": 200,
                "probability": 0.48,
                "is_stale": False,
                "is_calibrated": True,
            },
...
def compute_dynamic_weights(
    ...
    try:
        ...
        typer.echo(
            f"Warning: Configs not found in {config_dir}, using defaults.", err=True
        )
        policy_data = {
            "name": "default_policy",
            ...
            "max_weight_cap": 0.8,
            "trust_component_weight": 1.0,
            "regime_component_weight": 1.0,
            "disagreement_penalty_weight": 1.0,
            "recency_penalty_weight": 1.0,
    ...
    context = {
        "event_id": f"evt_{sport}_{market}_1",
        "sport": sport,
        "market_type": market,
    }
    ...
    if diagnostics.stale_penalties:
        typer.echo(
            f"  - Stale penalties applied: {', '.join(diagnostics.stale_penalties)}"
        )
...
def run_ensemble(
    ...
    sources = _get_mock_eligible_sources(sport, market)
    context = {
        "event_id": f"evt_{sport}_{market}_2",
        "sport": sport,
        "market_type": market,
    }
    ...
    from sports_signal_bot.ensemble.contracts import (
        EnsembleInputRecord, StandardizedPredictionRecord)
    ...
    for i, s in enumerate(sources):
        ...
        preds.append(
            StandardizedPredictionRecord(
                ...
                metadata=(
                    {
                        "dynamic_weight": w_record.final_weight,
                        "dynamic_weight_explanation": w_record.explanation_summary,
                    }
                    if w_record
                    else {}
                ),
    ...
    input_rec = EnsembleInputRecord(
        event_id=context["event_id"], sport=sport, market_type=market, predictions=preds
    )

    from sports_signal_bot.ensemble.runner import EnsembleRunner

    config = {"strategy": "dynamic_weighted_average", "strategy_config": {}}
