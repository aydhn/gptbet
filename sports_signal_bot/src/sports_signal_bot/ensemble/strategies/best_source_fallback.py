...
class BestSourceFallbackEnsembler(BaseEnsembler):
    ...
    def __init__(self, name: str = "best_source_fallback", config: Dict[str, Any] = None):
    ...
    def combine(self, input_record: EnsembleInputRecord) -> EnsembleOutputRecord:
        ...
        for priority_source in self.source_priority:
            for p in aligned_preds:
                if p.source_name == priority_source or p.source_family == priority_source:
                    selected_pred = p
        ...
        excluded = [p.source_name for p in aligned_preds if p.source_name != used_source_name]
