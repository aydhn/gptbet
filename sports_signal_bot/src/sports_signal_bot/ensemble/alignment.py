...
def validate_market_compatibility(source_classes: List[str], target_classes: List[str]) -> bool:
    """Check if source classes can be aligned to target classes."""
    return set(source_classes).issubset(set(target_classes))

def align_probability_vector(probs: Dict[str, float], source_classes: List[str], target_classes: List[str]) -> Optional[Dict[str, float]]:
...
def align_predictions_to_reference_classes(predictions: List['StandardizedPredictionRecord'], reference_classes: List[str]) -> List['StandardizedPredictionRecord']:
    """Aligns multiple prediction records to a reference class list."""
    from .contracts import StandardizedPredictionRecord
    aligned_preds = []

    for p in predictions:
        aligned_probs = align_probability_vector(p.probabilities, p.class_labels, reference_classes)
...
def drop_incompatible_sources(predictions: List['StandardizedPredictionRecord'], reference_classes: List[str]) -> List['StandardizedPredictionRecord']:
    """Returns only predictions that are compatible with the reference classes."""
    return [p for p in predictions if validate_market_compatibility(p.class_labels, reference_classes)]
