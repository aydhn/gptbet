...
class EnsembleRunner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.ensembler = EnsembleFactory.create(self.strategy_name, self.config.get("strategy_config", {}))
    ...
    def run(self, input_records: List[EnsembleInputRecord]) -> Dict[str, Any]:
        ...
        for record in input_records:
            # Apply policy
            filtered_preds = apply_calibrated_preference_policy(record.predictions, self.preference_mode)
    ...
    def _build_manifest(self, run_id: str, results: List[EnsembleOutputRecord]) -> EnsembleRunManifest:
        if not results:
             return EnsembleRunManifest(run_id=run_id, sport="unknown", market_type="unknown", ensemble_strategy=self.strategy_name, target_classes=[], config=self.config)
        ...
        for r in results:
            for src in r.component_sources:
                 source_summary[src.source_name] = source_summary.get(src.source_name, 0) + 1
        ...
        return EnsembleRunManifest(
            ...
            target_classes=list(results[0].final_probabilities.keys()) if results else [],
            config=self.config,
