...
def generate_availability_summary(
    ...
    counts = {name: len(df.columns) - 1 for name, df in builder_outputs.items()} # -1 for event_id
    ...
    out_file = Path(output_path).with_name(f"{Path(output_path).stem}_availability.json")
