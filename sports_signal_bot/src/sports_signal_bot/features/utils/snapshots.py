...
def select_feature_snapshot(events_df: pd.DataFrame, odds_df: pd.DataFrame, policy: str) -> pd.DataFrame:
    """
    Selects the most appropriate pre-match odds snapshot, avoiding post-event leakage.
    Returns a DataFrame keyed by event_id.
    """
    if odds_df.empty or "snapshot_time_utc" not in odds_df.columns or "event_datetime_utc" not in events_df.columns:
        return odds_df.copy()

    merged = pd.merge(odds_df, events_df[["event_id", "event_datetime_utc"]], on="event_id", how="inner")
    ...
    if len(valid_snapshots) < len(odds_df):
        logger.warning(f"Filtered out {len(odds_df) - len(valid_snapshots)} post-event odds snapshots to prevent leakage.")
    ...
    return latest_valid.drop(columns=["snapshot_dt", "event_dt", "event_datetime_utc"], errors="ignore")
