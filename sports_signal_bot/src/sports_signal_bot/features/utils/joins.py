...
def safe_merge(left: pd.DataFrame, right: pd.DataFrame, on: str, how: str = 'left', validate: Optional[str] = None) -> pd.DataFrame:
    """
    ...
    if final_rows != initial_rows and how in ['left', 'inner']:
        logger.warning(f"Row count changed during '{how}' merge on '{on}'. Initial: {initial_rows}, Final: {final_rows}")
