...
class FeatureSetAssembler:
    ...
    def add_feature_set(self, builder_name: str, df: pd.DataFrame):
        ...
        if 'event_id' not in df.columns:
            logger.warning(f"Builder {builder_name} output missing 'event_id'. Skipping.")
            return

        # Ensure event_id is string and no duplicates
        df['event_id'] = df['event_id'].astype(str)
        if df['event_id'].duplicated().any():
            logger.warning(f"Builder {builder_name} produced duplicate event_ids. Keeping first.")
    ...
    def assemble(self, null_policy: NullPolicy = NullPolicy.KEEP_NULLS) -> pd.DataFrame:
        ...
        for name in builder_names[1:]:
            ...
            base_df = pd.merge(base_df, df_to_join, on='event_id', how='outer', suffixes=('', f'_{name}'))
        ...
        if null_policy == NullPolicy.FILL_DEFAULTS:
            base_df = apply_null_policy(base_df, null_policy) # Basic fill, in real prod might be column-specific
