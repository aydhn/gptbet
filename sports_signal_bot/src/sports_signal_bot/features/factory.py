...
from sports_signal_bot.features.availability import \
...
class FeatureFactory:
    ...
    def build_feature_matrix(
        ...
        if not active_builders:
            logger.warning(
                f"No builders selected for sport {context.sport}. Returning empty matrix."
            )
            return pd.DataFrame()
        ...
        for builder in active_builders:
            ...
            try:
                ...
                missing_inputs = [
                    inp for inp in builder.required_inputs if inp not in data
                ]
                if missing_inputs:
                    logger.warning(
                        f"Builder {builder.name} skipped. Missing inputs: {missing_inputs}"
                    )
    ...
    def _get_active_builders(self, context: FeatureBuildContext):
        ...
        if context.include_feature_families:
            all_builders = [
                b for b in all_builders if b.family in context.include_feature_families
            ]
        ...
        if context.exclude_feature_families:
            all_builders = [
                ...
                if b.family not in context.exclude_feature_families
            ]
