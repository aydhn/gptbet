...
class LabelGenerator:
    def __init__(self, market_definitions: List[MarketDefinition], line_sets: Optional[Dict[str, List[float]]] = None):
    ...
    def generate(self, result: EventResultRecord) -> List[LabelRecord]:
        ...
        for m_type, market_def in self.markets.items():
            ...
            if not lines: lines = [None] # Ensure at least one attempt if no lines defined

            for line in lines:
                label_name = generate_label_name(m_type, line)

                try:
                    target_text, status, reason = resolve_market(market_def.settlement_rule_name, result, line)
                    class_idx = None
                    if target_text and market_def.selection_schema and target_text in market_def.selection_schema:
