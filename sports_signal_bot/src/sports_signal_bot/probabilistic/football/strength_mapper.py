...
from sports_signal_bot.probabilistic.football.contracts import LambdaBuildContext
...
class StrengthMapper:
    ...
    def map_strengths(self, context: LambdaBuildContext, features: Dict[str, float]) -> Tuple[float, float, float]:
        """
        ...
        # 1. Base Total Proxy
        base_total = features.get("league_total_goal_baseline", config.default_league_goal_baseline)
        if base_total <= 0:
        ...
        home_defense = features.get("home_defense_weakness", 1.0) # >1 means weak defense
