...
from sports_signal_bot.probabilistic.football.contracts import GoalLambdaEstimate, ScoreMatrixRecord, GoalEnvironmentConfig
...
class PoissonScoreMatrix:
    ...
    def __init__(self, lambda_estimate: GoalLambdaEstimate, config: GoalEnvironmentConfig):
    ...
    def _build(self):
        ...
        home_probs = poisson.pmf(np.arange(self.max_goals + 1), self.estimate.home_lambda)
        away_probs = poisson.pmf(np.arange(self.max_goals + 1), self.estimate.away_lambda)
        ...
        if truncated_mass > 0.05:
            warnings.append(f"High truncated mass warning: {truncated_mass:.4f} probability lost due to max_goals={self.max_goals}")
