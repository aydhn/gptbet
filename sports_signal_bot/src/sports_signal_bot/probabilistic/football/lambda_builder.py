...
from sports_signal_bot.probabilistic.football.contracts import (
    GoalLambdaEstimate, LambdaBuildContext)
from sports_signal_bot.probabilistic.football.strength_mapper import \
    StrengthMapper
...
class GoalLambdaBuilder:
    ...
    def build(
        ...
        if "final_home_score" in features or "final_away_score" in features:
            warnings.append(
                "LEAKAGE_WARNING: Final score present in features. Ignoring for lambda estimation."
            )

        # 2. Extract strengths and baseline
        home_str, away_str, base_total = self.strength_mapper.map_strengths(
            context, features
        )

        # 3. Apply Home Advantage
        home_advantage = features.get(
            "home_advantage", context.config.home_advantage_goal_bonus
        )
        ...
        home_lambda = self._clip_lambda(
            raw_home_lambda, context.config.lambda_min, context.config.lambda_max
        )
        away_lambda = self._clip_lambda(
            raw_away_lambda, context.config.lambda_min, context.config.lambda_max
        )

        if home_lambda != raw_home_lambda:
            warnings.append(
                f"Home lambda clipped from {raw_home_lambda:.2f} to {home_lambda:.2f}"
            )
        if away_lambda != raw_away_lambda:
            warnings.append(
                f"Away lambda clipped from {raw_away_lambda:.2f} to {away_lambda:.2f}"
            )
