...
from sports_signal_bot.probabilistic.basketball.contracts import (
    BasketballDistributionConfig, BasketballModelDiagnostics)
...
class BasketballDistributionCore:
    ...
    def prob_total_over(
        self, line: float, expected_total: float, total_std: float
    ) -> float:
        """P(Total > line)."""
        p = norm.sf(line, loc=expected_total, scale=total_std)
        return self._safe_prob(p)

    def prob_total_under(
        self, line: float, expected_total: float, total_std: float
    ) -> float:
        """P(Total < line)."""
    ...
    def prob_home_cover(
        self, spread_line: float, expected_margin_home: float, margin_std: float
    ) -> float:
        """
