...
from sports_signal_bot.probabilistic.basketball.contracts import BasketballScoreEstimate, BasketballDistributionConfig

class ExpectedPointsBuilder:
    """Builds expected points for home and away teams based on structural features."""

    def build(self, event_id: str, features: Dict[str, Any], config: BasketballDistributionConfig, model_name: str = "basketball_structural_points_model") -> BasketballScoreEstimate:
        warnings = []
        ...
        home_off_vs_away_def = features.get("home_off_vs_away_def", 0.0) * config.offense_weight
        away_off_vs_home_def = features.get("away_off_vs_home_def", 0.0) * config.defense_weight
        ...
        exp_home = base_home + (pace_adjustment / 2) + home_off_vs_away_def + (rating_diff_adj / 2)
        exp_away = base_away + (pace_adjustment / 2) + away_off_vs_home_def - (rating_diff_adj / 2)
