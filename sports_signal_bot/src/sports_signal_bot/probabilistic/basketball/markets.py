...
class BasketballMarketExtractor:
    ...
    def extract_moneyline(self, expected_margin_home: float, margin_std: float) -> Dict[str, float]:
    ...
    def extract_totals(self, expected_total: float, total_std: float, lines: List[float] = None) -> Dict[str, Dict[str, float]]:
    ...
    def extract_spreads(self, expected_margin_home: float, margin_std: float, lines: List[float] = None) -> Dict[str, Dict[str, float]]:
        """
        ...
        for line in lines:
            ...
            p_home_cover = self.core.prob_home_cover(line, expected_margin_home, margin_std)
            p_away_cover = self.core.prob_away_cover(line, expected_margin_home, margin_std)
            results[key] = {
