...
def test_totals_over_under_complementarity():
    ...
    totals = extractor.extract_totals(
        expected_total=220.0, total_std=10.0, lines=[220.0, 210.5, 230.5]
    )
    ...
    assert (
        pytest.approx(totals["total_210_5"]["over"] + totals["total_210_5"]["under"])
        == 1.0
    )
    ...
    assert (
        pytest.approx(totals["total_230_5"]["over"] + totals["total_230_5"]["under"])
        == 1.0
    )
