...
def test_preferred_calibrated_policy():
    policy = PreferredCalibratedPolicy(
        SourcePolicyDefinition(policy_name="PreferredCalibratedPolicy")
    )

    meta1 = SourceMetadataRecord(
        source_name="raw", event_id="e", sport="s", market_type="m", is_calibrated=False
    )
    meta2 = SourceMetadataRecord(
        source_name="cal", event_id="e", sport="s", market_type="m", is_calibrated=True
    )

    elig1 = SourceEligibilityRecord(
        event_id="e",
        sport="s",
        market_type="m",
        source_name="raw",
    ...
    assert any(
        ex.reason_code == "replaced_by_calibrated_variant"
        for ex in elig1.exclusion_reasons
    )
