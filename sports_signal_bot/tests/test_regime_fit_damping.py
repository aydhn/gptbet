...
from sports_signal_bot.source_selection.metadata import SourceMetadataRecord, RegimeProfileMetadata
...
def test_regime_fit_damping():
    ...
    meta_high = SourceMetadataRecord(
        ...
        regime_profile=RegimeProfileMetadata(regime_scores={"r1": 0.9}, regime_sample_sizes={"r1": 100})
    )
    ...
    meta_low = SourceMetadataRecord(
        source_name="s", event_id="e", sport="s", market_type="m",
        regime_profile=RegimeProfileMetadata(regime_scores={"r1": 0.9}, regime_sample_sizes={"r1": 10})
    )
