...
def test_runner_basic():
    config = BankrollConfig(default_overlay_strategy=OverlayStrategyName.FLAT_STAKE, flat_stake_units=100.0)
    ...
    decisions = [
        BankrollDecisionRecord(
            ...
            event_datetime_utc=now, action_class="approved",
            executed_flag=True, result_status="settled_win", hit_flag=True, payout_multiple=1.0
        ),
        BankrollDecisionRecord(
            event_id="e2", market_type="m", sport="s",
            event_datetime_utc=now + datetime.timedelta(hours=1), action_class="approved",
            executed_flag=True, result_status="settled_loss", hit_flag=False, payout_multiple=1.0
        )
    ]
