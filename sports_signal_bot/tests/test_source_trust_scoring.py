import math
from sports_signal_bot.source_selection.metadata import SourceMetadataRecord, EvaluationMetadata, RefreshMetadata
...
def test_trust_scoring_performance():
    ...
    meta_good = SourceMetadataRecord(source_name="s", event_id="e", sport="s", market_type="m",
                                     eval_info=EvaluationMetadata(recent_log_loss=0.4))
    assert scorer.compute_performance_component(meta_good) == 1.0

    meta_bad = SourceMetadataRecord(source_name="s", event_id="e", sport="s", market_type="m",
                                     eval_info=EvaluationMetadata(recent_log_loss=0.8))
    assert scorer.compute_performance_component(meta_bad) == 0.0

    meta_mid = SourceMetadataRecord(source_name="s", event_id="e", sport="s", market_type="m",
                                     eval_info=EvaluationMetadata(recent_log_loss=0.6))
    assert math.isclose(scorer.compute_performance_component(meta_mid), 0.5, rel_tol=1e-9)
