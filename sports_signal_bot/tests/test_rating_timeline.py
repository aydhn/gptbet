...
def test_timeline_ordering_and_update():
    ...
    # Out of order events
    events = [
        CanonicalEventRecord(event_id="e2", sport=SportType.FOOTBALL, league="L", season="S",
            event_datetime_utc=datetime(2023, 1, 2, 12, 0), home_team="A", away_team="C", status="completed", source="m", source_event_id="m"),
        CanonicalEventRecord(event_id="e1", sport=SportType.FOOTBALL, league="L", season="S",
            event_datetime_utc=datetime(2023, 1, 1, 12, 0), home_team="A", away_team="B", status="completed", source="m", source_event_id="m"),
    ]
    results = [
        EventResultRecord(event_id="e2", sport=SportType.FOOTBALL, status="completed", final_home_score=0, final_away_score=1),
        EventResultRecord(event_id="e1", sport=SportType.FOOTBALL, status="completed", final_home_score=1, final_away_score=0),
    ]
...
def test_cancelled_events_skipped():
    ...
    events = [
        CanonicalEventRecord(event_id="e1", sport=SportType.FOOTBALL, league="L", season="S",
            event_datetime_utc=datetime(2023, 1, 1, 12, 0), home_team="A", away_team="B", status="cancelled", source="m", source_event_id="m"),
    ]
    results = [
        EventResultRecord(event_id="e1", sport=SportType.FOOTBALL, status="cancelled", final_home_score=None, final_away_score=None),
    ]
