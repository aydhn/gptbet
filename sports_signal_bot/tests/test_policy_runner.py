...
from sports_signal_bot.policy.contracts import PolicySignalStatus, ActionClass
...
def test_approved_signal(base_config):
    ...
    signal = SignalPolicyInputRecord(
        ...
        final_probability=0.8, final_signal_score=0.9,
        edge_estimate=0.05, status=SignalStatus.SCORED,
        components_summary={
...
def test_no_bet_zone_gray(base_config):
    ...
    signal = SignalPolicyInputRecord(
        event_id="e1", sport="football", market_type="1x2", selection="home",
        final_probability=0.5, final_signal_score=0.45,
        edge_estimate=0.02, status=SignalStatus.SCORED,
        components_summary={
...
def test_candidate_not_approved(base_config):
    ...
    signal = SignalPolicyInputRecord(
        event_id="e1", sport="football", market_type="1x2", selection="home",
        final_probability=0.7, final_signal_score=0.75, # candidate > 0.6 but not approved < 0.8
        edge_estimate=0.04, status=SignalStatus.SCORED,
        components_summary={
            "uncertainty_penalty": 0.05,
            "disagreement_penalty": 0.1,
            "data_quality_penalty": 0.0,
