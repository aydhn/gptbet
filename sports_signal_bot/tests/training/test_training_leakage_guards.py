import pytest
import pandas as pd
from sports_signal_bot.training.leakage import detect_suspicious_feature_columns, audit_feature_target_alignment, enforce_pre_match_only_feature_policy
from sports_signal_bot.core.exceptions import LeakageDetectedError

def test_detect_suspicious_columns():
    cols = ['home_rating', 'away_rating', 'final_score_home', 'target_variable', 'weather']
...
def test_audit_target_in_features():
    ...
    with pytest.raises(LeakageDetectedError):
        audit_feature_target_alignment(df, ['feat_1', 'actual_outcome'], 'actual_outcome')
