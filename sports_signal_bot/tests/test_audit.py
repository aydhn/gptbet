...
def test_detect_leakage():
    ...
    audit_pass = detect_post_event_snapshot_leakage(
        event_start, pre_match_ts, "1", "1x2"
    )
    assert audit_pass.audit_status == "pass"

    audit_fail = detect_post_event_snapshot_leakage(
        event_start, post_match_ts, "1", "1x2"
    )
