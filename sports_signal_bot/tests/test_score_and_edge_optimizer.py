import pytest
from sports_signal_bot.thresholds.strategies.score_and_edge import ScoreAndEdgeThresholdOptimizer
...
def test_score_and_edge_optimizer():
    ...
    acc, rej = optimizer.apply_threshold(signals, {"score_threshold": 0.5, "edge_threshold": 0.1})
    assert len(acc) == 1
    assert len(rej) == 1
