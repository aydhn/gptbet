...
def test_dynamic_hybrid_strategy():
    policy = WeightingPolicyDefinition(name="test", description="desc", min_weight_floor=0.1, max_weight_cap=0.9)
    strategy = DynamicHybridWeighted(policy, {"family_priors": {"default": {}}})

    sources = [
        {"name": "a", "family": "f1", "trust_score": 0.9, "regime_fit": 0.9, "regime_sample_size": 100, "is_calibrated": True},
        {"name": "b", "family": "f2", "trust_score": 0.5, "regime_fit": 0.5, "regime_sample_size": 20, "is_stale": True},
    ]
    context = {"event_id": "1", "sport": "s", "market_type": "m"}
...
def test_runner_fallback():
    runner = DynamicWeightingRunner("dynamic_hybrid", {"name": "p", "description": "d"}, {})
