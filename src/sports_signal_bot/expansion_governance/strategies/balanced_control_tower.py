...
class BalancedControlTowerStrategy(BaseExpansionGovernanceStrategy):
    """
    Balanced strategy.
    Standard thresholds, uses selective rollbacks frequently instead of global pauses.
    """
    ...
    def evaluate_state(self, metrics: Dict[str, Any]) -> ExpansionGovernanceManifest:
        ...
        if 'budget_usage' in metrics:
            budgets.append(ExpansionBudgetRecord(
                ...
                budget_status="healthy" if metrics['budget_usage'] < 0.7 else ("exhausted" if metrics['budget_usage'] > 0.95 else "warning")
            ))
