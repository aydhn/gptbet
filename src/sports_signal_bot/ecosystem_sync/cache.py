...
class RoutingCacheBuilder:
    ...
    def build_routing_cache(self, query_family: str, routing_record: EcosystemRoutingRecord) -> RoutingCacheRecord:
        ...
        cache_record = RoutingCacheRecord(
            ...
            fallback_candidates=[c.candidate_ref for c in routing_record.candidate_refs if c.candidate_ref not in routing_record.selected_route_refs]
        )
        self._cache[query_family] = cache_record
        return cache_record

    def invalidate_stale_routes(self, query_family: str, reason: str) -> Optional[RoutingInvalidationRecord]:
        ...
        if query_family in self._cache:
            ...
            return RoutingInvalidationRecord(
                invalidation_id=f"inv_{datetime.utcnow().timestamp()}",
                reason=reason
            )
        return None

    def update_cache_after_overlay_rebuild(self, query_family: str, new_routing: EcosystemRoutingRecord) -> None:
    ...
    def summarize_cache_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stale_queries": sum(1 for c in self._cache.values() if c.freshness_state == "stale"),
            "fresh_queries": sum(1 for c in self._cache.values() if c.freshness_state == "fresh")
        }

    def detect_sync_vs_cache_drift(self, query_family: str, latest_sync_timestamp: datetime) -> bool:
