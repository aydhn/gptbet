...
from .contracts import (
    PolicyEvaluationRecord, PolicyDecisionRecordV2, PolicyDecisionStatus,
    PolicyBundleRecord, PolicyRuleRecord, PolicyOverlayRecord
)
...
class PolicyResolver:
    ...
    def resolve_relevant_policies(self, context: Dict[str, Any], target_bundle_ids: List[str]) -> List[PolicyRuleRecord]:
        relevant_rules = []
...
class PolicyEvaluator:
    ...
    def evaluate_policy_conditions(self, rules: List[PolicyRuleRecord], context: Dict[str, Any]) -> List[PolicyRuleRecord]:
        matched_rules = []
        for rule in rules:
            all_match = True
            for condition in rule.conditions:
                if not self.evaluate_condition(condition, context):
                    all_match = False
                    break
            if all_match or not rule.conditions: # Empty conditions means it always applies if scoped correctly
...
class PolicyDecisionBuilder:
    ...
    def execute_policy_actions(self, rules: List[PolicyRuleRecord]) -> Dict[str, Any]:
        ...
        rule_families = [r.rule_family for r in rules]
        ...
        for rule in sorted_rules:
            for action in rule.actions:
                ...
                if action_type == "deny":
                    result["status"] = PolicyDecisionStatus.DENY
                    result["blockers"].append(f"Blocked by {rule.rule_id}: {rule.title}")
                elif action_type == "require_approval" and result["status"] != PolicyDecisionStatus.DENY:
                     result["status"] = PolicyDecisionStatus.REQUIRE_APPROVAL
                     result["followups"].append(f"Approval required by {rule.rule_id}")
                elif action_type == "escalate":
                     result["status"] = PolicyDecisionStatus.ESCALATE
                     result["escalations"].append(f"Escalated by {rule.rule_id}")
                elif action_type == "hold" and result["status"] not in [PolicyDecisionStatus.DENY, PolicyDecisionStatus.ESCALATE]:
                     result["status"] = PolicyDecisionStatus.HOLD
                     result["blockers"].append(f"Held by {rule.rule_id}: {rule.title}")

        return result

    def build_policy_decision(self, matched_rules: List[PolicyRuleRecord], target_bundle_ids: List[str], context_id: str) -> PolicyDecisionRecordV2:
...
class PolicyEngine:
    ...
    def evaluate(self, context: Dict[str, Any], target_bundle_ids: List[str]) -> PolicyEvaluationRecord:
        relevant_rules = self.resolver.resolve_relevant_policies(context, target_bundle_ids)
        matched_rules = self.evaluator.evaluate_policy_conditions(relevant_rules, context)
        context_id = context.get("context_id", f"ctx_{uuid.uuid4().hex[:8]}")
        decision = self.decision_builder.build_policy_decision(matched_rules, target_bundle_ids, context_id)
