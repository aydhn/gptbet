...
def save_artifact(filename: str, data: Any):
    ...
    with open(filepath, "w") as f:
        if isinstance(data, list):
            dump_data = [item.model_dump() if hasattr(item, "model_dump") else item for item in data]
...
def run_hardening_pass(strategy_name: str) -> Dict[str, Any]:
    ...
    from src.sports_signal_bot.geo_quorum_hardening.passive_checkpoints import summarize_passive_state
    from src.sports_signal_bot.geo_quorum_hardening.coverage_seams import summarize_coverage_seams
    from src.sports_signal_bot.geo_quorum_hardening.summaries import build_geo_quorum_operational_matrix

    save_artifact("passive_readiness_report.json", [summarize_passive_state({})])
    save_artifact("coverage_seam_report.json", [summarize_coverage_seams({})])
    save_artifact("geo_quorum_operational_matrix.json", build_geo_quorum_operational_matrix({}))


    # 5. Budgets
    budget = summarize_geo_quorum_budgets({
        "quorum_erosion_breach": False
    })
    ...
    manifest_inputs = {
        ...
        "coverage_seam_gaps": 1 if coverage.synthesis_status != "coverage_synthesized" else 0,
    ...
    health_report = {
        ...
        "release_blockers": len([w for w in manifest.warnings if "blocked" in manifest.overall_status]),
