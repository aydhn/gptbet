...
from sports_signal_bot.execution_coordination.contracts import (
    ExecutionCoordinationFabricRecord,
    FabricStatus,
    CoordinationHealthRecord,
    PriorityBand,
    SchedulingWindowRecord,
)
...
class SupervisedExecutionCoordinationFabric:
    ...
    def admit_lane(
        ...
        requested_window: SchedulingWindowRecord,
    ):
        self.ledger.append_entry(
            lane_ref, "lane_submitted", {"priority": priority_band}
        )
        schedule = self.scheduler.submit_lane(lane_ref, priority_band, requested_window)
        self.ledger.append_entry(
            lane_ref, "schedule_requested", {"schedule_id": schedule.schedule_id}
        )
        return schedule

    def request_allocation(self, lane_ref: str, scope: str):
        self.ledger.append_entry(
            lane_ref, "broker_reservation_requested", {"scope": scope}
        )
        decision = self.broker.request_allocation(lane_ref, scope)
        if decision.granted:
            self.ledger.append_entry(
                lane_ref, "token_allocated", {"decision_id": decision.decision_id}
            )
        else:
            self.ledger.append_entry(
                lane_ref,
    ...
    def coordinate(self):
        ...
        new_contentions = self.contention_detector.detect_contentions(
            waiting_lane_refs, shared_surfaces
        )
        for contention in new_contentions:
            ...
            # 2. Arbitrate
            decision = self.arbitration_engine.arbitrate(
                contention, self.scheduler.active_schedules
            )
            for ref in decision.winning_lane_refs:
                self.ledger.append_entry(
                    ref,
                    "arbitration_decided",
                    {"outcome": "win", "decision_id": decision.arbitration_id},
                )
            for ref in decision.deferred_lane_refs:
                self.ledger.append_entry(
                    ref,
                    "arbitration_decided",
                    {"outcome": "deferred", "decision_id": decision.arbitration_id},
                )
            ...
            self.contention_detector.resolve_contention(
                contention.contention_id, decision.arbitration_id
            )

        # 3. Process remaining waiting schedules
        for i in range(len(self.scheduler.waiting_schedules) - 1, -1, -1):
            s = self.scheduler.waiting_schedules[i]
        ...
        active_contentions_count = len(
            [
                c
                for c in self.contention_detector.active_contentions
                if c.current_resolution_state == "unresolved"
            ]
        )
        self.status = self.strategy.compute_fabric_status(
            active_contentions_count, 0.0, self.broker.health_status
        )
    ...
    def get_summary(self) -> ExecutionCoordinationFabricRecord:
        return ExecutionCoordinationFabricRecord(
            ...
            contention_refs=[
                c.contention_id for c in self.contention_detector.active_contentions
            ],
            arbitration_refs=[
                d.arbitration_id for d in self.arbitration_engine.arbitration_history
            ],
