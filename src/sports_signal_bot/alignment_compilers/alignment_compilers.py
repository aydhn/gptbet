...
from .contracts import (
    ...
    AlignmentPassRecord,
    AlignmentDimensionRecord,
    AlignmentPenaltyRecord,
    AlignmentOutputRecord,
...
def register_alignment_input(
    ...
    if input_record.currentness_state == "stale":
         compiler.warnings.append(GovernanceAlignmentCompilerWarningRecord("stale_input", f"Input {input_record.alignment_input_id} is stale."))
    if input_record.no_safe_visibility_state == "hidden":
         compiler.warnings.append(GovernanceAlignmentCompilerWarningRecord("no_safe_hidden", f"Input {input_record.alignment_input_id} hides no-safe visibility."))
...
def execute_alignment_compiler_passes(
    ...
    for inp in inputs:
        if inp.sovereignty_state == "failed":
            ...
            break
    passes.append(AlignmentPassRecord(pass_id="pass_sov_1", pass_type="sovereignty_preservation_pass", result=sovereignty_pass_result))
    ...
    for inp in inputs:
        if inp.no_safe_visibility_state == "hidden":
            ...
            break
    passes.append(AlignmentPassRecord(pass_id="pass_no_safe_1", pass_type="no_safe_visibility_pass", result=no_safe_pass_result))

    compiler.pass_refs.extend([p.pass_id for p in passes])
    return passes

def summarize_alignment_compiler(compiler: SovereignGovernanceAlignmentCompilerRecord) -> Dict[str, str]:
...
def compute_alignment_dimensions(inputs: List[AlignmentInputRecord]) -> List[AlignmentDimensionRecord]:
    """Computes alignment dimensions based on inputs."""
    dimensions = []
    dimensions.append(AlignmentDimensionRecord(dimension_type="context_alignment", score=0.8))
    dimensions.append(AlignmentDimensionRecord(dimension_type="sovereignty_preservation", score=1.0))
    return dimensions

def apply_alignment_penalties(inputs: List[AlignmentInputRecord], passes: List[AlignmentPassRecord]) -> List[AlignmentPenaltyRecord]:
    """Determines alignment penalties."""
    penalties = []
    for inp in inputs:
        if inp.currentness_state == "stale":
            penalties.append(AlignmentPenaltyRecord(
...
def classify_alignment_penalties(penalties: List[AlignmentPenaltyRecord]) -> Dict[str, int]:
...
def attach_alignment_penalty_explanations(penalties: List[AlignmentPenaltyRecord]) -> List[str]:
    """Generates explanations for the applied penalties."""
    return [f"{p.penalty_family} ({p.severity}): {p.reason}" for p in penalties]

def summarize_alignment_penalty_pressure(penalties: List[AlignmentPenaltyRecord]) -> str:
