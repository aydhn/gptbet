...
from .contracts import (
    ...
    GuardOrderingRecord, GuardTimeoutRecord, GuardCancellationRecord, ConcurrencyGuardWarningRecord,
    ConcurrencyGuardHealthRecord, ConcurrencyGuardManifestRecord
)
...
def build_concurrency_guard(
    ...
    if guard_family not in [
        "shared_state_guard", "async_join_guard", "ordering_guard",
        "stale_read_guard", "idempotency_guard", "queue_bound_guard",
        "write_once_guard", "side_effect_guard"
    ]:
        status = "guard_review_only"
        warnings.append(
            ConcurrencyGuardWarningRecord(
                warning_id=f"warn_{uuid.uuid4().hex[:8]}",
                message=f"Unknown guard family: {guard_family}",
                severity="medium"
            )
        )

    if timeout_ms > 30000: # Example rule
         status = "guard_caveated"
         warnings.append(
            ConcurrencyGuardWarningRecord(
                warning_id=f"warn_{uuid.uuid4().hex[:8]}",
                message=f"Timeout unusually high: {timeout_ms}ms",
                severity="low"
            )
        )
    ...
    return ConcurrencyGuardRecord(
        ...
        protected_surface_ref=protected_surface_ref,
...
def validate_guard_coverage(guards: List[ConcurrencyGuardRecord], surfaces: List[str]) -> List[str]:
...
def detect_unprotected_shared_state(guards: List[ConcurrencyGuardRecord], known_states: List[str]) -> List[str]:
    """Detects shared state surfaces that lack a 'shared_state_guard'."""
    shared_state_guards = {g.protected_surface_ref for g in guards if g.guard_family == "shared_state_guard"}
    return [s for s in known_states if s not in shared_state_guards]

def summarize_concurrency_guards(guards: List[ConcurrencyGuardRecord]) -> ConcurrencyGuardManifestRecord:
    """Summarizes concurrency guards into a manifest."""
    unhealthy_guards = [g.concurrency_guard_id for g in guards if g.guard_status not in ["guard_safe", "guard_caveated"]]
