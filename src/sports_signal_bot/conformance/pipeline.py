...
from .specs import AssertionRegistry, GovernanceSpecRegistry
...
class ContinuousVerificationRunner:
    ...
    def run_pipeline(
        ...
        stages.append(
            VerificationStageRecord(
                stage_name="spec_resolution",
                status="success",
                details="Specs resolved.",
            )
        )
        ...
        stages.append(
            VerificationStageRecord(
                stage_name="lint_stage",
                status="success" if lint_result.passed else "failure",
                details=f"Lint passed: {lint_result.passed}",
            )
        )
        ...
        stages.append(
            VerificationStageRecord(
                stage_name="static_conformance",
                status="success" if all_passed else "failure",
                details=f"Conformance passed: {all_passed}",
            )
        )

        # 4. Drift
        drift_result = self.drift_runner.run_drift_attestation(
            current_state, expected_state
        )
        stages.append(
            VerificationStageRecord(
                stage_name="drift_attestation",
                status="success",
                details=f"Drift outcome: {drift_result.outcome.value}",
            )
        )
        ...
        gate = self.gate_evaluator.evaluate_gate(input_record)
        stages.append(
            VerificationStageRecord(
                stage_name="gate_evaluation",
                status="success",
                details=f"Gate outcome: {gate.outcome.value}",
            )
        )

        return VerificationPipelineRunRecord(
            run_id="run_01", run_mode=mode, stages=stages, final_outcome=gate.outcome
        )
...
def run_pipeline_stage(
    stage_name: str, runner: ContinuousVerificationRunner, context: Dict
) -> VerificationStageRecord:
    return VerificationStageRecord(
        stage_name=stage_name, status="success", details="Run stage"
    )
...
def compute_compliance_outcome(
    run_record: VerificationPipelineRunRecord,
) -> GateOutcome:
    return run_record.final_outcome


def emit_pipeline_artifacts(
    run_record: VerificationPipelineRunRecord,
) -> ComplianceManifest:
    ...
    return ComplianceManifest(
        manifest_id="man_01", run_id=run_record.run_id, decision=decision
    )
