...
from .contracts import PortalQueryRecord, PortalResultRecord, QuerySafetyDecisionRecord
from .profiles import get_profile

def validate_portal_query(query: PortalQueryRecord) -> QuerySafetyDecisionRecord:
    if query.query_type == "raw_internal":
        return QuerySafetyDecisionRecord(safe=False, reason="Arbitrary raw internal queries are not allowed")
    if "internal_path" in query.params or "key" in query.params:
        return QuerySafetyDecisionRecord(safe=False, reason="Sensitive internal path or key lookups are not allowed")
...
def redact_query_results(results: Dict[str, Any], profile_id: str) -> Dict[str, Any]:
    ...
    if profile.signer_metadata_masking_level == "full":
        ...
        for item in redacted.get("items", []):
            if "signer_metadata" in item:
                item["signer_metadata"] = {k: v for k, v in item["signer_metadata"].items() if k in ["signer_id", "timestamp"]}
    return redacted
...
def execute_profile_aware_query(query: PortalQueryRecord) -> PortalResultRecord:
    ...
    raw_data = {"status": "ok", "items": [{"id": 1, "data": "test", "signer_metadata": {"key": "secret", "signer_id": "s1", "timestamp": "now"}}]}

    redacted_data = redact_query_results(raw_data, query.profile)

    return PortalResultRecord(
        result_id=f"res_{query.query_id}",
...
def summarize_query_response(result: PortalResultRecord) -> Dict[str, Any]:
    return {"result_id": result.result_id, "item_count": len(result.data.get("items", []))}
