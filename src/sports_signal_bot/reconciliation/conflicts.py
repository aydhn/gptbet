...
from sports_signal_bot.reconciliation.contracts import ReconciliationGroupRecord, FieldConflictRecord


def detect_conflicts(group: ReconciliationGroupRecord) -> List[FieldConflictRecord]:
    conflicts = []
    ...
    for field in all_fields:
        ...
        if len(distinct_vals) > 1:
            severity = "medium"
            if field in ["status", "final_score"]: severity = "critical"

            conflicts.append(FieldConflictRecord(
