...
from sports_signal_bot.consistency_ledgers.tribunal_meshes import compute_tribunal_mesh_pressure
from sports_signal_bot.consistency_ledgers.clearing_routes import (
    enumerate_clearing_matches,
    score_clearing_matches,
    apply_clearing_constraints,
    select_clearing_outcome
)
from sports_signal_bot.consistency_ledgers.evidence_clearers import compute_clearing_pressure
from sports_signal_bot.consistency_ledgers.ledger_entries import register_consistency_entry
from sports_signal_bot.consistency_ledgers.contradictions import detect_consistency_contradictions
...
def build_alignment_ledger_pipeline(
    ...
    currentness = compute_federated_alignment_currentness(federation, nodes, links)
    agreement = compute_alignment_federation_agreement(federation, nodes, currentness)
    penalties, ceilings = preserve_penalties_and_ceilings_in_alignment_federation(federation, nodes)

    decision = aggregate_federated_alignment_outputs(federation, currentness, agreement, penalties, ceilings)
    no_safe = preserve_no_safe_visibility_in_alignment_federation(federation, nodes)

    # Map decision to ledger consistency state
    cons_state = ConsistencyState.CONSISTENT_WITH_CAPS
    if decision.decision_output == FederatedAlignmentOutputStatus.FEDERATED_ALIGNMENT_STALE:
        cons_state = ConsistencyState.STALE_CONSISTENCY
    elif decision.decision_output == FederatedAlignmentOutputStatus.FEDERATED_ALIGNMENT_DEGRADED:
        cons_state = ConsistencyState.DEGRADED_CONSISTENCY
    elif decision.decision_output == FederatedAlignmentOutputStatus.FEDERATED_ALIGNMENT_CAVEATED:
    ...
    entry = register_consistency_entry(
        ledger,
        LedgerEntryFamily.CONTEXT_CONSISTENCY_ENTRY,
        federation.alignment_federation_id,
        federation.federation_family.value,
        "current" if currentness.is_current else "stale",
        cons_state,
        caveats
    )
...
def build_tribunal_mesh_context_pipeline(
    ...
    constrained = apply_tribunal_mesh_constraints(scored, pressure.pressure_state)
...
def build_clearer_consistency_pipeline(
    ...
    for book_id in clearer.clearing_book_refs:
        if book_id in books:
            ...
            constrained = apply_clearing_constraints(scored, "high" if pressure.pressure_score > 50 else "low")
