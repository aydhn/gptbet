...
class ConservativeConsistencyLedgerStrategy(BaseConsistencyLedgerStrategy):
    ...
    def apply_federation_rules(self, federation: AlignmentCompilerFederationRecord) -> AlignmentCompilerFederationRecord:
        ...
        federation.health_status = HealthStatus.DEGRADED if len(federation.active_link_refs) == 0 else HealthStatus.HEALTHY
        return federation
    ...
    def apply_clearing_rules(self, clearer: EvidenceExchangeClearerRecord) -> EvidenceExchangeClearerRecord:
        clearer.warnings.append("Conservative: Cleared outputs fast-tracked to caveated/stale")
        return clearer
