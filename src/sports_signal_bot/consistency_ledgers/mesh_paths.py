...
def score_tribunal_mesh_paths(
    ...
    for path in paths:
        ...
        for edge_id in path:
            ...
            source_node = nodes.get(edge.source_node_ref)
            target_node = nodes.get(edge.target_node_ref)

            if target_node and target_node.backlog_state in ["high", "critical"]:
                score -= 30
                if outcome in [TribunalMeshRouteOutcome.BOUNDED_TRIBUNAL_ROUTE, TribunalMeshRouteOutcome.CAVEATED_TRIBUNAL_ROUTE]:
...
def apply_tribunal_mesh_constraints(
    ...
    for p in scored_paths:
        if pressure_state == MeshPressureState.REVIEW_ONLY_BIAS:
            if p["outcome"] in [TribunalMeshRouteOutcome.BOUNDED_TRIBUNAL_ROUTE, TribunalMeshRouteOutcome.CAVEATED_TRIBUNAL_ROUTE]:
                p["outcome"] = TribunalMeshRouteOutcome.REVIEW_ONLY_TRIBUNAL_ROUTE
                p["warnings"].append("Downgraded to review_only due to mesh pressure bias.")
             ...
             if p["outcome"] != TribunalMeshRouteOutcome.NO_SAFE_TRIBUNAL_ROUTE:
                 ...
                 p["warnings"].append("Blocked due to critical mesh pressure suppression.")
    return scored_paths
