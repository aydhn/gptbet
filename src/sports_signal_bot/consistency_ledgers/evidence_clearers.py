...
def compute_clearing_pressure(
    ...
    stale_listings = sum(1 for l in listings.values() if l.currentness_state != "current")
...
def compute_clearing_fairness(
    clearer: EvidenceExchangeClearerRecord,
    pressure: ClearingPressureRecord
) -> ClearingFairnessRecord:
    metrics = {
        "request_aging": 10.0,
        "review_only_spillover": 0.2
    }
