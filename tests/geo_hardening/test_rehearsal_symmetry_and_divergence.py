...
def test_compute_rehearsal_symmetry():
    assert compute_rehearsal_symmetry([]) == {
        "symmetry_ratio": 1.0,
        "is_asymmetric": False,
    }
    assert compute_rehearsal_symmetry([1.0, 1.05]) == {
        "symmetry_ratio": 1.025,
        "is_asymmetric": False,
    }
    assert compute_rehearsal_symmetry([1.0, 1.5]) == {
        "symmetry_ratio": 1.25,
        "is_asymmetric": True,
    }
    assert compute_rehearsal_symmetry([1.0]) == {
        "symmetry_ratio": 1.0,
        "is_asymmetric": False,
    }
    assert compute_rehearsal_symmetry([2.0, 2.0, 2.0]) == {
        "symmetry_ratio": 2.0,
        "is_asymmetric": False,
    }
    assert compute_rehearsal_symmetry([-1.0, -1.05]) == {
    ...
    assert compute_rehearsal_symmetry([0.0, 0.0]) == {
        "symmetry_ratio": 0.0,
        "is_asymmetric": False,
    }
    assert compute_rehearsal_symmetry([1.0, 1.05, 1.2]) == {
        "symmetry_ratio": 1.0833333333333333,
        "is_asymmetric": True,
    }
...
def test_detect_dual_writer_risk():
    ...
    assert not detect_dual_writer_risk(
        [{"region": "us-east"}, {"is_write": True}]
    )


def test_summarize_rehearsal_divergence():
    assert summarize_rehearsal_divergence([0.6, 0.2, 0.1]) == {
        "total_divergences": 3,
        "critical": 1,
    }
    assert summarize_rehearsal_divergence([]) == {
        "total_divergences": 0,
        "critical": 0,
    }
    assert summarize_rehearsal_divergence([0.1, 0.2, 0.3]) == {
        "total_divergences": 3,
        "critical": 0,
    }
    assert summarize_rehearsal_divergence([0.6, 0.7]) == {
        "total_divergences": 2,
        "critical": 2,
    }
