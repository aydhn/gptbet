...
def test_build_geo_resilience_budgets():
    ...
    assert budgets == {"lag_budget_seconds": 300, "asymmetry_budget_percentage": 1}
...
def test_measure_geo_budget_consumption_healthy():
    ...
    assert result == {"status": "healthy", "breaches": []}
...
def test_measure_geo_budget_consumption_breached():
    ...
    result = measure_geo_budget_consumption(budget, consumption)
    assert result == {"status": "breached", "breaches": ["lag_budget_breached"]}
...
def test_measure_geo_budget_consumption_missing_keys():
    ...
    assert result_breach == {"status": "breached", "breaches": ["lag_budget_breached"]}


def test_summarize_geo_resilience_budgets_healthy():
    consumption_result = {"status": "healthy", "breaches": []}
    summary = summarize_geo_resilience_budgets(consumption_result)
    assert summary == {"budget_status": "healthy", "breach_count": 0}


def test_summarize_geo_resilience_budgets_breached():
    consumption_result = {"status": "breached", "breaches": ["lag_budget_breached"]}
    summary = summarize_geo_resilience_budgets(consumption_result)
    assert summary == {"budget_status": "breached", "breach_count": 1}


def test_summarize_geo_resilience_budgets_missing_keys():
    consumption_result = {}
    summary = summarize_geo_resilience_budgets(consumption_result)
    assert summary == {"budget_status": "unknown", "breach_count": 0}
