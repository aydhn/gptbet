...
from sports_signal_bot.adjudication.contracts import (KnowledgeScopeRecord,
                                                      KnowledgeScopeType)
...
def test_scope_safety():
    ...
    invalid_scope = ScopeManager.build_knowledge_scope(
        scope_type=KnowledgeScopeType.single_entity, target_value=""
    )
    assert not ScopeManager.validate_scope_safety(invalid_scope)

    global_advisory = ScopeManager.build_knowledge_scope(
        scope_type=KnowledgeScopeType.global_advisory_only, target_value=""
    )
    ...
    assert not AdjudicationGuardrails.check_memory_scope_not_overbroad(
        global_auto_apply
    )
